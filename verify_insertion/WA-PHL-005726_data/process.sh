...
bamfile=WA_PHL.removereads.bam
samtools view -@ 32 -h $bamfile > $samfile
noclipsam=$(basename $samfile .sam).noclip.sam
noclipbam=$(basename $noclipsam .sam).bam
# remove primer soft clipping from read sequences
python ../remove_sam_softclip.py $samfile nCoV-2019_v3.bed $noclipsam
samtools view -@ 32 -b $noclipsam | samtools sort -n -o $noclipbam
samtools collate -u -O $noclipbam | samtools fastq -@ 32 -1 paired1.fq -2 paired2.fq -0 /dev/null -s /dev/null -n
